...
class RatingModelOptimizer:
    ...
    def _build_model(self, trial):
        ...
        if combined_type == 'dot_product':
            ...
            # Dense para expandir expressividade (dot_product produz apenas 2 escalares)
        ...
        output = tf.keras.layers.Dense(1, activation='tanh', bias_initializer=output_bias_init)(combined)
    ...
    def train_final_model(self, study=None, model_save_path='final_model.h5', history_save_path='history.pkl'):
        ...
        print(f"Teste final - Loss(Huber): {test_loss:.4f} | "
              f"MAE: {test_mae:.4f} ({test_mae * scale:.4f} na escala original) | "
              f"RMSE: {test_rmse:.4f} ({test_rmse * scale:.4f} na escala original) | "
              f"R²: {test_r2:.4f}")
...
if __name__ == "__main__":
    ...
    study = optimizer.optimize(
        n_trials=100,
        save_path='best_study_01_reg_pca.pkl',
    ...
    model, predictions = optimizer.train_final_model(
        study,
        model_save_path='data/output/final_model_01_reg_pca.h5',
        history_save_path='data/output/history_01_reg_pca.pkl'
    )

    print("================\nPREDICTING\n================")
    print(predictions.sample(20))
    predictions.to_csv("data/output/predictions_01_reg_pca.csv", index=False)
    print("Regression ==")
